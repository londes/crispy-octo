...
export default function TodoList({ todos, complete, remove, edit, change, dragStart, dragEnter, dragEnd }) {
  return (
    <ul>
        {todos.map((todo, idx) => <li className='todo-li' key={todo._id} index={idx} draggable onDragStart={(e) => dragStart(e, idx)} onDragEnter={(e) => dragEnter(e, idx)} onDragEnd={dragEnd} onDragOver={(e)=>e.preventDefault()}> 
            ...
            {/* { todo.editing
                ...
                : <div className='todo-todo' style={todo.completed ? styles.complete : styles.incomplete}>{todo.todo}</div> }
            <div className='todo-button-container'>
                <button className='todo-button' onClick={edit} idx={idx}>{todo.editing ? 'done' : 'edit'}</button>
                <button className='todo-button' onClick={complete} idx={idx}>{todo.completed ? 'incomplete' : 'complete'}</button>
                <button className='todo-button' onClick={remove} idx={idx}>remove</button>
            </div> */}
        </li>)}
